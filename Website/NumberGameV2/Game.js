...
function renderGuesses(){
    document.querySelector(`.guess-table`).innerHTML = ' <tr><th>Guess no</th><th>Guess</th><th>Correct Digit</th><th>Correct Position</th></tr>';
    for(let i = 0; i < guess.length; i++){
        let currHtml = document.querySelector(`.guess-table`).innerHTML;
        currHtml += `<tr><td>Guess ${i + 1}:</td><td> ${MakeDigitsEqualToNumber(guess[i].guess)}</td><td>${guess[i].correctDigits}</td><td>${guess[i].correctPosition}</td></tr>`;
        document.querySelector(`.guess-table`).innerHTML = currHtml;
    }
}//render guesses    
